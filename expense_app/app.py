...
def require_login():
    ...
    st.title("התחברות")
    st.write("האפליקציה פרטית. כדי להיכנס יש להקליד סיסמה.")
    pwd = st.text_input("סיסמה", type="password")

    app_pwd = st.secrets.get("APP_PASSWORD", "")
    if not app_pwd or "CHANGE_ME" in app_pwd:
        st.error("צריך להגדיר סיסמה בקובץ .streamlit/secrets.toml (APP_PASSWORD).")
        st.stop()
...
def ocr_image_bytes(img_bytes: bytes) -> str:
    ...
    config = "--oem 3 --psm 6"
    return pytesseract.image_to_string(thr, lang="eng", config=config)
...
def main():
    ...
    with tab1:
        ...
        with st.expander("הגדרות חילוץ (רשות)", expanded=False):
            st.caption("אם הזיהוי לא מדויק, אפשר תמיד לתקן ידנית לפני שמירה.")
            use_ocr_for_pdf = st.checkbox("אם PDF סרוק, להשתמש ב-OCR (איטי יותר)", value=False)
        ...
        if uploaded is not None:
            ...
            if is_pdf:
                ...
                if (not extracted_text.strip()) and use_ocr_for_pdf:
                    st.warning("PDF נראה סרוק ואין בו טקסט לחילוץ. במקרה כזה עדיף להזין ידנית את התאריך והסכום.")
                    extracted_text = ""
            else:
                try:
                    extracted_text = ocr_image_bytes(file_bytes)
                except Exception:
                    st.error("לא הצלחתי לבצע OCR לתמונה. אפשר להוסיף ידנית.")
                    extracted_text = ""
            ...
            if save:
                if not doc_date:
                    st.error("חסר תאריך. אפשר להזין ידנית (YYYY-MM-DD).")
                else:
                    try:
        ...
        if df.empty:
            ...
            df_show = df.copy()
            df_show["month"] = df_show["doc_date"].dt.to_period("M").astype(str)
            df_show["amount_fmt"] = df_show["amount_ils"].apply(ils)
            ...
            st.dataframe(df_show[["id","חודש","תאריך","סכום","ספק","קטגוריה","הערות","קובץ","נוצר"]], use_container_width=True, hide_index=True)
            ...
            if row.empty:
                ...
                with st.form("edit_form"):
                    ...
                    with c1:
                        new_date = st.text_input("תאריך (YYYY-MM-DD)", value=r["doc_date"].date().isoformat() if pd.notna(r["doc_date"]) else "")
                    with c2:
    ...
    with tab2:
        ...
        with left:
            ...
            st.dataframe(monthly[["month","סהכ הוצאות"]].rename(columns={"month":"חודש"}), use_container_width=True, hide_index=True)
